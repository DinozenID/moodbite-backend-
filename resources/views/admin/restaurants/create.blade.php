...
<div class="glass-panel rounded-2xl overflow-hidden max-w-4xl mx-auto">
    <div class="p-6 border-b border-slate-700/50 bg-slate-800/30">
        <h2 class="text-lg font-semibold text-slate-100">Restaurant Details</h2>
        <p class="text-sm text-slate-400 mt-1">Add a new restaurant with geolocation.</p>
    </div>
        ...
        <form action="{{ route('admin.restaurants.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <textarea name="address" rows="3" required
                              class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-2 text-slate-100 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors">{{ old('address') }}</textarea>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 border-t border-slate-700/50 pt-6">
                ...
                <button type="submit" class="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors flex items-center">
                    <i data-lucide="save" class="w-4 h-4 mr-2"></i> Save Restaurant
                </button>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let initialLng = 101.6869;
        
        let map = L.map('map').setView([initialLat, initialLng], 12);
        ...
        // Check if there are old values (validation fail)
        ...
        // Fix map rendering issue in tabs/hidden divs by invalidating size
