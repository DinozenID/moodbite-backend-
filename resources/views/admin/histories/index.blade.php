...
@section('title', 'Manage Histories')
@section('header', 'Manage Histories')

@section('content')
<div class="glass-panel rounded-2xl overflow-hidden">
    <div class="p-6 border-b border-slate-700/50 flex justify-between items-center bg-slate-800/30">
        <h2 class="text-lg font-semibold text-slate-100">User History</h2>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-slate-300 text-sm divide-y divide-slate-700/50">
                ...
                <tr class="hover:bg-slate-800/30 transition-colors">
                    <td class="p-4">{{ $item->id ?? $item->history_id }}</td>
                    <td class="p-4">History Record</td>
                </tr>
                @empty
                <tr>
                    <td colspan="2" class="p-8 text-center text-slate-400">
                        <i data-lucide="history" class="w-12 h-12 mx-auto mb-3 opacity-20"></i>
                        No history found.
                    </td>
                </tr>
                @endforelse
